...
export default function HomePage() {
  ...
  if (eventId) {
    ...
    const dateStr = evt.startDate
      ? new Date(evt.startDate).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
      : 'TBA';
          ...
          <section className="event-banner-section">
            <div className="banner-flex-container">
              <div className="banner-poster-col">
                {evt.bannerUrl ? (
                  <img src={evt.bannerUrl} alt={evt.name} className="event-poster-img" />
                ) : (
                  <div style={{
                    width: '100%', height: '260px', borderRadius: '0.75rem',
                ...
                <div className="event-title-container">
                  ...
                  <div className="share-dropdown-wrapper">
                    ...
                    <div className={`share-dropdown-menu ${showShareMenu ? 'show' : ''}`}>
                      <button type="button" className="share-menu-item" onClick={() => { handleShareAction('copy', setToastMessage, evt.name); setShowShareMenu(false); }}>
                      ...
                      <button type="button" className="share-menu-item" onClick={() => { handleShareAction('whatsapp', setToastMessage, evt.name); setShowShareMenu(false); }}>
                      ...
                      <button type="button" className="share-menu-item" onClick={() => { handleShareAction('twitter', setToastMessage, evt.name); setShowShareMenu(false); }}>
                        <svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                      </button>
                      <button type="button" className="share-menu-item" onClick={() => { handleShareAction('linkedin', setToastMessage, evt.name); setShowShareMenu(false); }}>
                    ...
                    <div className="info-card">
                      ...
                      <div className="info-card-text">
                        ...
                        {typeof evt.venue === 'object' && evt.venue.mapsLink && (
                          <a href={evt.venue.mapsLink} target="_blank" rel="noopener noreferrer" style={{fontSize: '0.72rem', color: 'var(--brand-primary)', fontWeight: 600}}>View on Maps ↗</a>
                        )}
                      </div>
                    </div>
                  )}
